...
export async function sendVerificationCodeEmail({ to, name, code }: EmailOptions) {
  ...
  const mailOptions = {
    ...
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: auto;">
        ...
        <p>Click <a href="${process.env.NEXTAUTH_URL}/salon/register/verification?email=${encodeURIComponent(
          to
    ...
    text: `
      ...
      Visit ${process.env.NEXTAUTH_URL}/salon/register/verification?email=${encodeURIComponent(
        to
      )} to enter your code.\n\n
...
export async function sendPasswordResetEmail({ to, name, resetToken }: ResetEmailOptions) {
  // Validate environment variables
  if (!process.env.EMAIL_USER || !process.env.EMAIL_PASS) {
    throw new Error("Email configuration missing: EMAIL_USER or EMAIL_PASS not set");
  }
...
export async function sendPaymentConfirmationEmail({
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      await transporter.sendMail(mailOptions);
      console.log(`Payment confirmation email sent to ${to} on attempt ${attempt}`);
      return;
    } catch (error: any) {
      lastError = error;
      console.error(`Attempt ${attempt} failed to send email to ${to}:`, error);
  ...
  console.log(`Fallback: Payment confirmation for ${to}: ${planName}, ${planPrice}, ${billingCycle}`);
