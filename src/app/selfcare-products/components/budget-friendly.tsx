  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        if (result.success) {
          const budgetFriendly = result.data
            .filter(
              (product: Product) => product.category.name === "Budget Friendly"
            )
            .slice(0, 4)
            .map((product: Product) => ({
              ...product,
      ...
      <div className="px-2">
        <Link href="/selfcare-products" className="prose lg:prose-xl">
          <h3 className="mb-2 md:mb-3 relative text-secondary font-semibold text-lg bg-base-100">
            ...
            <span className="pl-4">Budget Friendly</span>
          </h3>
        </Link>
        <p className="text-gray-600">No budget-friendly products found.</p>
      </div>
    );
  }
