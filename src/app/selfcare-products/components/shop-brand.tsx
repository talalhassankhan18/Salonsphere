  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        if (result.success) {
          const brandProducts = result.data
            .filter(
              (product: Product) => product.category.name === "Loreal Products"
            )
            .slice(0, 4)
            .map((product: Product) => ({
              ...product,
      ...
      <div className="px-2">
        <Link href="/selfcare-products" className="prose lg:prose-xl">
          <h3 className="mb-2 md:mb-3 relative text-secondary font-semibold text-lg bg-base-100">
            ...
            <span className="pl-4">Shop By Brand</span>
          </h3>
        </Link>
        <h5 className="mb-2 md:mb-3 relative text-secondary font-semibold text-base bg-base-100">
          <span className="pl-4">L'Oréal Products</span>
        </h5>
        <p className="text-gray-600">No L'Oréal products found.</p>
      </div>
    );
  }
