...
interface Product {
  ...
  price: number;
  stock: number;
  attributes: { attributeId: { name: string; values: string[] }; value: string }[];
  imageUrls: string[];
  description: string;
  howToUse: string;
...
const HotDeals = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        if (result.success) {
          const hotDeals = result.data
            .filter((product: Product) => product.category.name === "Hot Deals")
            .slice(0, 4)
            .map((product: Product) => ({
              ...product,
              ...
              status: ["active", "inactive", "out-of-stock"].includes(product.status)
                ? product.status
                : ("active" as "active" | "inactive" | "out-of-stock"),
            }));
      ...
      <div className="px-2">
        <Link href="/selfcare-products" className="prose lg:prose-xl">
          <h3 className="mb-2 md:mb-3 relative text-secondary font-semibold text-lg bg-base-100">
            ...
            <span className="pl-4">Hot Deals</span>
          </h3>
        </Link>
        <p className="text-gray-600">No Hot Deals found.</p>
      </div>
    );
  }
  ...
  const handleAddToCart = (product: Product) => {
    const productType: ProductType = cardData.find((p) => p._id === product._id)!;
