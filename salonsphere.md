...
The reported `Failed to fetch salons (500)` overlays came from `/api/salon` returning 500 because
**MongoDB had crashed** (`mongod.log`: `terminate() called. An exception is active`, right after
"saving checkpoint"). The Windows `MongoDB` service was STOPPED. Underneath that: **drive C: had
0.0 GB free**. WiredTiger cannot write a checkpoint on a full disk, and `next build` also failed
with `ENOSPC` and a Node "Zone Allocation failed" OOM.

- [x] MongoDB service restarted (UAC elevation).
- [x] Freed ~7.9 GB: deleted `.next/` (2.0 GB, regenerated by every build) and
      `npm cache clean --force` (5.8 GB). `dbConnect` recovers on its own once Mongo is back.
- [x] `console.error` → `console.warn` in the salon fetch `catch` blocks (home, `/salons`,
      nearby, recommended-products). Next 15 dev turns every `console.error` into the red
      "Console Error" overlay even though those components already render a friendly error
      state. Diagnostics stay in the console; the overlay is gone.
- [ ] **You:** keep ≥ 5 GB free on C:. Consider setting the MongoDB service to auto-restart on
      failure (admin prompt): `sc failure MongoDB reset= 86400 actions= restart/5000/restart/10000/restart/30000`
